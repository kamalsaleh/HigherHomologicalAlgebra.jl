...
@InstallMethod( DerivedCategoryByCochains,
  ...
  function ( CAP_NAMED_ARGUMENTS, cat )
    ...
    homotopy_cat = HomotopyCategoryByCochains( cat; overhead = overhead );
    
    derived_cat = CreateCapCategory( name, IsDerivedCategoryByCochains, IsDerivedCategoryByCochainsObject, IsDerivedCategoryByCochainsMorphism, IsCapCategoryTwoCell; overhead = overhead );
    
    derived_cat.category_as_first_argument = true;
    
    SetDefiningCategory( derived_cat, cat );
    
    SetUnderlyingCategory( derived_cat, HomotopyCategoryByCochains( cat; overhead = overhead ) );
    ...
    AddIsEqualForMorphisms( derived_cat,
      function( derived_cat, alpha, beta )
        ...
        return IsEqualForMorphisms( pair_alpha[1], pair_beta[1] ) && IsEqualForMorphisms( pair_alpha[2], pair_beta[2] );
    ...
    AddIsWellDefinedForMorphisms( derived_cat,
      function( derived_cat, alpha )
        ...
        return IsEqualForObjects( Source( pair[1] ), Source( pair[2] ) ) && ForAll( pair, IsWellDefined ) && IsQuasiIsomorphism( UnderlyingCell( pair[1] ) );
    ...
    if (HasIsAbelianCategoryWithEnoughProjectives( cat ) && IsAbelianCategoryWithEnoughProjectives( cat ))
      
      ADD_EXTRA_FUNCTIONS_TO_DERIVED_CATEGORY_VIA_LOCALIZATION_BY_PROJECTIVE_OBJECTS( derived_cat );
      
    elseif (HasIsAbelianCategoryWithEnoughInjectives( cat ) && IsAbelianCategoryWithEnoughInjectives( cat ))
...
@InstallGlobalFunction( ADD_EXTRA_FUNCTIONS_TO_DERIVED_CATEGORY_VIA_LOCALIZATION_BY_PROJECTIVE_OBJECTS,
  function ( derived_cat )
    ...
    if (HasRangeCategoryOfHomomorphismStructure( homotopy_cat ))
      
      range_cat = RangeCategoryOfHomomorphismStructure( homotopy_cat );
      
      if (HasIsAbelianCategory( range_cat ) && IsAbelianCategory( range_cat ))
    ...
    if (CanCompute( homotopy_cat, "BasisOfExternalHom" ) && CanCompute( homotopy_cat, "CoefficientsOfMorphism" )
          && HasRangeCategoryOfHomomorphismStructure( homotopy_cat ))
