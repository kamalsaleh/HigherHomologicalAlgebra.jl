...
@InstallMethod( EmbeddingIntoDerivedCategory,
  ...
  function( homotopy_cat )
    ...
    if (@not IsAdditiveClosureCategory( DefiningCategory( homotopy_cat ) ))
        TryNextMethod( );
    end;
    
    add_closure = DefiningCategory( homotopy_cat );
    
    if (@not IsAdditiveClosureCategory( add_closure ))
        TryNextMethod( );
    end;
     
    B = UnderlyingCategory( add_closure );
    
    if (!( IsFpAlgebroid( B ) && HasRangeCategoryOfHomomorphismStructure( B ) ))
        TryNextMethod( );
    end;
  ...
  function ( seq )
    ...
    AddMorphismFunction( T,
      function ( TF, alpha, TG )
        ...
        if (@IsBound( TF.defining_data_for_tensor_functor ))
        ...
        if (@IsBound( TG.defining_data_for_tensor_functor ))
  ...
  function( seq )
    ...
    AddNaturalTransformationFunction( epsilon,
      function ( TH_A, A, ID_A )
        ...
        if (@IsBound( TH_A.defining_data_for_tensor_functor ))
  ...
  function ( seq )
    ...
    AddNaturalTransformationFunction( mu,
      function ( ID_F, F, HT_F )
        ...
        if (@IsBound( HT_F.defining_data_for_hom_functor ))
            
            T_F = HT_F.defining_data_for_hom_functor.argument;
            
        else T_F = ApplyFunctor( T, F ); end;
