                ...
                List( (1):(nr_vertices - 1),
                    i -> @Concatenation( List( (i + 1):(nr_vertices),
                        j -> List( (1):(Length( IrreducibleMorphisms( seq, [ i, j ] ) )),
                            k -> [ @Concatenation( "m", StringGAP(i), "_", StringGAP(j), "_", StringGAP(k) ),
                                   i,
                                   j,
                                   @Concatenation( "m_", LATEX_LBRACE, StringGAP(i), ",", StringGAP(j), LATEX_RBRACE, "^", LATEX_LBRACE, StringGAP(k), LATEX_RBRACE )
                                 ] ) ) ) ) );
    
    q = FinQuiver(
            @Concatenation(
                "q(",
                JoinStringsWithSeparator( List( (1):(nr_vertices), i -> @Concatenation( "E", StringGAP( i ) ) ), "," ),
                ")[", JoinStringsWithSeparator( List( arrows, m -> @Concatenation( m[1], ":", "E", StringGAP(m[2]), "->", "E", StringGAP(m[3]) ) ), "," ), "]" ) );
    
    vertices_latex = List( (1):(nr_vertices), i -> @Concatenation( "E_", LATEX_LBRACE, StringGAP(i), LATEX_RBRACE ) );
