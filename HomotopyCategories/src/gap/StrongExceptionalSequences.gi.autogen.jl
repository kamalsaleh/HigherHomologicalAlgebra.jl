  ...
  function ( list_of_objects )
    ...
    if (@not ForAll( (1):(Length( list_of_objects ) - 1),
                    i -> ForAll( (i + 1):(Length( list_of_objects )),
                      j -> IsZeroForObjects( range_cat, HomomorphismStructureOnObjects( ambient_cat, list_of_objects[j], list_of_objects[i] ) ) ) ))
...
@InstallMethod( DataOfYonedaEmbeddingOnObjectRelativeToStrongExceptionalSequence,
  ...
  function ( seq, A )
    ...
    if (@not IsIdenticalObj( ambient_cat, CapCategory( A ) ))
    ...
    return PairGAP( object_func, morphism_func );
...
@InstallMethod( DataOfYonedaEmbeddingOnMorphismRelativeToStrongExceptionalSequence,
  ...
  function ( seq, alpha )
    ...
    if (@not IsIdenticalObj( ambient_cat, CapCategory( alpha ) ))
...
@InstallMethod( PossibileExceptionalShifts,
  ...
  function( seq, A )
    ...
    if (@not IsIdenticalObj( AmbientCategory( seq ), CapCategory( A ) ))
...
@InstallMethod( MaximalExceptionalShift,
  ...
  function( seq, A )
    ...
    for j in (1):(Length( possible_shifts ))
      
      shift_A = ApplyShift( A, possible_shifts[j] );
      
      for i in (1):(nr_objects)
        
        if (@not IsZeroForObjects( HomomorphismStructureOnObjects( ambient_cat, UnderlyingCell( seq[i] ), shift_A ) ))
  ...
  function ( seq, A, is_bounded )
    ...
    if (is_bounded == false)
      ...
      while true
        ...
        if (IsEqualForObjects( GA[i], zero_object ) && MaximalExceptionalShift( seq, Range( data[i][2] ) ) == -infinity)
            break;
        end;
