...
julia> objects = [ [ "A", [ -5, 5 ] ], [ "B", [ -2, 2 ] ], [ "C", [ -2, 2 ] ], [ "D", [ -5, 5 ] ] ];

julia> morphisms = [ [ "x", [ 1, 2 ], -1, [ -1, 3 ], "x" ], [ "alpha", [ 1, 2 ], 0, [ -2, 2 ], "\\alpha" ], [ "y", [ 3, 4 ], -1, [ -2, 2 ], "y" ], [ "beta", [ 3, 4 ], 0, [ -2, 2 ], "\\beta" ] ];

julia> relations = [  [ "alpha - Differential( x )", 1 ], [ "Differential( alpha )", 1 ], [ "beta - Differential( y )", 3 ], [ "Differential( beta )", 3 ] ];
...
julia> C = CreateComplex( Ch_AAoid,
                   List( [ oid.dC_m2, oid.dC_m1, oid.dC_0, oid.dC_1 ],
                         m -> m / Aoid / AAoid ),
                   -2 );

julia> D = CreateComplex( Ch_AAoid,
...
julia> beta = CreateComplexMorphism( Ch_AAoid, C,
                      List( [ oid.beta_m2, oid.beta_m1, oid.beta_0, oid.beta_1, oid.beta_2 ],
                            m -> m / Aoid / AAoid ),
                      -2, D );

julia> qA = QuasiIsomorphismFromProjectiveResolution( A, true );

julia> qB = QuasiIsomorphismFromProjectiveResolution( B, true );

julia> Aq =  QuasiIsomorphismIntoInjectiveResolution( A, true );

julia> Bq =  QuasiIsomorphismIntoInjectiveResolution( B, true );

julia> prj_res_alpha = MorphismBetweenProjectiveResolutions( alpha, true );

julia> inj_res_alpha  = MorphismBetweenInjectiveResolutions( alpha, true );

julia> @Assert( 0, ForAll( [ qA, qB, Aq, Bq, prj_res_alpha, inj_res_alpha ], IsWellDefined ) )

julia> @Assert( 0, PreCompose( qA, alpha ) == PreCompose( prj_res_alpha, qB ) && PostCompose( Bq, alpha ) == PostCompose( inj_res_alpha, Aq ) )
