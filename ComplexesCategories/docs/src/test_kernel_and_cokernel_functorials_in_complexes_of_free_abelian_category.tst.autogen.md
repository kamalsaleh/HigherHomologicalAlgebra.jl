...
julia> U = CreateComplex( Ch_AAoid,
...
julia> V = CreateComplex( Ch_AAoid,
...
julia> alpha = CreateComplexMorphism( Ch_AAoid, A,
                      [ oid.alpha_m3 / Aoid / AAoid, oid.alpha_m2 / Aoid / AAoid ],
                      -3, B );

julia> f = CreateComplexMorphism( Ch_AAoid, A,
                  [ oid.f_m3 / Aoid / AAoid, oid.f_m2 / Aoid / AAoid, oid.f_m1 / Aoid / AAoid ],
                  -3, U );

julia> nu = CreateComplexMorphism( Ch_AAoid, U,
...
julia> g = CreateComplexMorphism( Ch_AAoid, B,
                  [ oid.g_m4 / Aoid / AAoid, oid.g_m3 / Aoid / AAoid, oid.g_m2 / Aoid / AAoid,
                    oid.g_m1 / Aoid / AAoid, oid.g_0 / Aoid / AAoid, oid.g_1 / Aoid / AAoid ],
                  -4, V );

julia> @Assert( 0, ForAll( [ f, g, alpha, nu ], IsWellDefined ) && IsZeroForMorphisms( PreCompose( alpha, g ) - PreCompose( f, nu ) ) )

julia> lambda = KernelObjectFunctorial( alpha, f, nu );

julia> @Assert( 0, IsWellDefined( lambda ) && IsZeroForMorphisms( PreCompose( KernelEmbedding( alpha ), f ) - PreCompose( lambda, KernelEmbedding( nu ) ) ) )

julia> lambda = CokernelObjectFunctorial( alpha, g, nu );

julia> @Assert( 0, IsWellDefined( lambda ) && IsZeroForMorphisms( PreCompose( CokernelProjection( alpha ), lambda ) - PreCompose( g, CokernelProjection( nu ) ) ) )

julia> u = UniversalMorphismIntoFiberProduct( Ch_AAoid, [ g, nu ], A, [ alpha, f ] );

julia> @Assert( 0, IsWellDefined( u ) )

julia> u = UniversalMorphismFromPushout( Ch_AAoid, [ alpha, f ], V, [ g, nu ] );
